...
var defaults = {
	...
	columnFormat: {
		month: 'ddd',//REMOVED VALUE 'm/d'
		week: 'ddd',//REMOVED VALUE 'm/d'
		day: 'dddd'//REMOVED VALUE 'm/d'
	},
...
function Calendar(element, options, eventSources) {
	...
	// TODO: improve view switching (still weird transition in IE, and FF has white out problem)
	...
	function buildSkeleton() {
		...
		/* ALL DAY ROW REMOVED*/
			s =
				...
				"<table style='width:100%' class='fc-agenda-allday' id='abc' cellspacing='0'>" +
				"<tr>" +
		...
		eventElement.draggable({
			...
			stop: function(ev, ui) {
				...
				if (cell && (dayDelta || minuteDelta || allDay)) {
					// changed!
					var selectedSubName='';
					for(temp_sub=0; temp_sub<event.title.indexOf("["); temp_sub++){
						selectedSubName = selectedSubName+event.title[temp_sub];
					}
					var checkAssign = true;
					$('input[name="subName"]').each(function() {
						if(selectedSubName == this.value)
							checkAssign = false;
  			     });
  			     if(checkAssign){
  			     		alert("You have not been assigned the selected subject");
  			     		eventL();
  			     		return;
  			     }  			     
					...
					var temp_str4=event.title.indexOf("[")+1;
					var schId4='';
					while(temp_str4<event.title.indexOf("]")){
					schId4=schId4+event.title[temp_str4];
					temp_str4++;
					}
					$.ajax({
				 url: "scheduleUpdate?scheduleId="+schId4+"&minuteDelta="+minuteDelta+"&dayDelta="+dayDelta+"&event=drag",
				 type: "GET",
...
function View(element, calendar, viewName) {
	...
	*/
	...
	function eventResize(e, event, dayDelta, minuteDelta, ev, ui) {
		...
		var temp_str3=event.title.indexOf("[")+1;
					var schId3='';
					while(temp_str3<event.title.indexOf("]")){
					schId3=schId3+event.title[temp_str3];
					temp_str3++;
					}
					$.ajax({
				 url: "scheduleUpdate?scheduleId="+schId3+"&minuteDelta="+minuteDelta+"&dayDelta="+dayDelta+"&event=resize",
				 type: "GET",
				 success: function(data) {
				eventL();
				},
